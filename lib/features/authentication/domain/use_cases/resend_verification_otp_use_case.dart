import 'package:doublem/core/models/either/either.dart';
import 'package:doublem/core/models/errors/failure_model.dart';
import 'package:doublem/core/use_cases/generic_use_case.dart';
import 'package:doublem/features/authentication/data/models/requests_body_model/resend_verification_otp_request_body.dart';
import 'package:doublem/features/authentication/domain/repositories/authentication_reository.dart';

class ResendVerificationOtpUseCase
    extends UseCase<void, ResendVerificationOtpRequestBody> {
  final AuthRepository authRepository;

  ResendVerificationOtpUseCase({required this.authRepository});

  @override
  Future<Either<Failure, void>> call({
    ResendVerificationOtpRequestBody? parameters,
  }) {
    return authRepository.resendVerificationOtp(request: parameters!);
  }
}
